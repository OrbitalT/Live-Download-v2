...
//mp3
var locamp3 = fs.readFileSync('mp3loca.ldf', 'utf8');
console.log('MP3 file location set to ' + locamp3);
//mp4
var locamp4 = fs.readFileSync('mp4loca.ldf', 'utf8');
console.log('MP4 file location set to ' + locamp4);
//channel
var locachannel = fs.readFileSync('channelloca.ldf', 'utf8');
...
ipcMain.on('item:mp3loca', function(e, item) {
  fs.writeFile('mp3loca.ldf', item + '/%(title)s.%(ext)s ', (err) => {
    console.log('MP3 location updated ' + item);
  });
});
//mp4
ipcMain.on('item:mp4loca', function(e, item) {
  fs.writeFile('mp4loca.ldf', item + '/%(title)s.%(ext)s ', (err) => {
    console.log('MP4 location updated ' + item);
  });
});
//channel
ipcMain.on('item:chanlocanew', function(e, item) {
  fs.writeFile('channelloca.ldf', item + '/%(uploader)s/%(title)s.%(ext)s ', (err) => {
